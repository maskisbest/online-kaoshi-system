  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      ...
      <ul class="paper" v-loading="loading">
        ...
        >
          <div class="status-row">
            ...
            <el-tag class="tag" :type="statusInfo(item.status).type">
      ...
      <div
        class="empty"
        v-if="
          !loading && (!pagination.records || pagination.records.length === 0)
        "
      >
        暂无可参加的考试
      </div>
...
export default {
  ...
  methods: {
    ...
    enrichExam(item) {
      ...
      const status = this.getExamStatus(examDate, item.totalTime);
      return { ...item, examDate, status };
    },
    //获取当前所有考试信息
    getExamInfo() {
      this.$axios(
    ...
    statusInfo(status) {
      const map = {
        notStarted: { label: "未开始", type: "info" },
        ongoing: { label: "考试中", type: "success" },
        ended: { label: "已结束", type: "danger" },
        pending: { label: "未获取", type: "warning" },
      };
...
.paper ::v-deep .enter.el-button--primary {
...
.paper ::v-deep .enter.el-button--primary:hover {
  background: var(--brand-strong);
  border-color: var(--brand-strong);
}

.top ::v-deep .el-badge__content {
