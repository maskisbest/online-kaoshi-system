        ...
        <div class="left" v-if="slider_flag">
          ...
          <div class="l-bottom">
            ...
            <div class="item">
              ...
              <ul>
                <li v-for="(list, index4) in topic[4]" :key="index4">
                  <a
                    href="javascript:;"
                    @click="subject(index4)"
                    :class="{
                      border: index === index4 && currentType === 4,
                      bg: subjectStatus[index4] === true,
                    }"
                    ><span
            ...
            <div class="actions">
              ...
              <div class="final" @click="commit(examData.examCode)">
                结束考试
              </div>
            </div>
          </div>
      ...
      <transition name="slider-fade">
        <div class="right">
          ...
          <div class="content">
            ...
            <div class="subject" v-if="currentType == 4">
              <el-input
                type="textarea"
                :rows="6"
                v-model="subjectAnswer[index]"
                placeholder="请在此作答，提交后主观题按参考答案要点判分"
                resize="none"
                @input="handleSubjectInput"
              >
              </el-input>
              <p class="tips">
                提示：系统按参考答案要点进行基础判分，复杂题型建议教师人工复核。
              </p>
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                  ...
                  <li>{{ topic[4][index].answer || "暂无参考答案" }}</li>
                  <li>
                    <el-tag>题目解析：</el-tag>
                  </li>
                  <li>
...
export default {
  data() {
    return {
      ...
      subjectAnswer: [], //保存所有主观题答案
      subjectStatus: [], //主观题作答状态
  ...
  methods: {
    ...
    getExamData() {
      ...
      this.$axios(`/api/exam/${examCode}`).then((res) => {
        ...
        this.$axios(`/api/paper/${paperId}`).then((res) => {
          ...
          this.fillAnswer = (this.topic[2] || []).map((q) => {
            const blanks = Math.max(
              this.questionBlankCount(q.question),
              this.normalizeFillAnswers(q.answer).length,
              1
            );
          ...
          this.topic1Answer = [];
          this.judgeAnswer = [];
          this.subjectAnswer = (this.topic[4] || []).map(() => "");
          this.subjectStatus = this.subjectAnswer.map(() => false);
          this.reduceAnswer =
            (this.topic[1] && this.topic[1][this.index]) || {};
    ...
    change(index) {
      ...
      if (this.index < len) {
        ...
        this.index = 0;
        if ((this.topic[2] || []).length > 0) {
          this.fill(this.index);
        } else if ((this.topic[3] || []).length > 0) {
          this.judge(this.index);
        } else if ((this.topic[4] || []).length > 0) {
          this.subject(this.index);
        }
      }
    },
    fillBG() {
      ...
      if (Array.isArray(this.fillAnswer[this.index])) {
        const answered = this.fillAnswer[this.index].some(
          (item) => item !== null && item !== ""
        );
    ...
    handleSubjectInput() {
      const content = this.subjectAnswer[this.index];
      const answered =
        content !== null &&
        content !== undefined &&
        String(content).trim() !== "";
      this.$set(this.subjectStatus, this.index, answered);
      this.saveProgress(false);
    },
    fill(index) {
      //填空题
    ...
    judge(index) {
      ...
      if (this.index < len) {
        if (this.index < 0) {
          ...
          this.showQuestion = Data[index].question; //获取题目信息
          this.number =
            (this.topicCount[0] || 0) + (this.topicCount[1] || 0) + index + 1;
        }
      } else if (this.index >= len) {
        this.index = 0;
        if ((this.topic[4] || []).length > 0) {
          this.subject(this.index);
        } else {
          this.change(this.index);
        }
    ...
    subject(index) {
      //主观题
      const list = this.topic[4] || [];
      const len = list.length;
      this.currentType = 4;
      this.index = index;
      if (this.index < len) {
        if (this.index < 0) {
          const prevLen = (this.topic[3] || []).length;
          this.index = prevLen > 0 ? prevLen - 1 : 0;
          this.judge(this.index);
        } else {
          this.title = "请作答（主观题）";
          this.showQuestion = list[index].question;
          this.number =
            (this.topicCount[0] || 0) +
            (this.topicCount[1] || 0) +
            (this.topicCount[2] || 0) +
    ...
    mark() {
      ...
      switch (this.currentType) {
        ...
        case 3:
          ...
          break;
        case 4:
          this.topic[4][this.index]["isMark"] = true; //主观题标记
      }
    },
    ...
    restoreProgress() {
      ...
      try {
        ...
        this.subjectAnswer = parsed.subjectAnswer || this.subjectAnswer;
        ...
        this.fillStatus = this.fillAnswer.map(
          (item) =>
            Array.isArray(item) && item.some((v) => v !== null && v !== "")
        );
        this.subjectStatus = this.subjectAnswer.map(
          (item) =>
    ...
    scoreSubjectQuestion(question, answerText) {
      const keywords = this.normalizeSubjectKeywords(question.answer);
      const student = this.normalizeText(answerText);
      if (!student) return 0;
      if (keywords.length === 0) return 0;
      const passed = keywords.every((key) => student.includes(key));
      return passed ? Number(question.score || 0) : 0;
    },
    ...
    calculateObjectiveScore() {
      ...
      fillList.forEach((q, idx) => {
        ...
        const blanks = Math.max(
          expectAnswers.length,
          this.questionBlankCount(q.question)
        );
        const studentAnswers = (this.fillAnswer[idx] || [])
          .slice(0, blanks)
          .map((ans) => this.normalizeText(ans));
        const expected = expectAnswers.length
          ? expectAnswers
          : [this.normalizeText(q.answer)];
        let correctCount = 0;
    ...
    async gradeSubjectQuestions() {
      const subjectList = this.topic[4] || [];
      const tasks = subjectList.map((q, idx) => {
        const studentAnswer = this.subjectAnswer[idx];
        if (
        ...
        return this.$axios({
          ...
          .then((res) => {
            if (
              res.data &&
              res.data.data &&
              res.data.data.score !== undefined
            ) {
              return Number(res.data.data.score) || 0;
            }
            return this.scoreSubjectQuestion(q, studentAnswer);
          })
          .catch(() => {
            return this.scoreSubjectQuestion(q, studentAnswer);
          });
    ...
    async commit(id, forceSubmit = false) {
      //答案提交计算分数
      if (this.submitting) {
        return;
      }
      const objectiveScore = this.calculateObjectiveScore();
      const subjectScore = await this.gradeSubjectQuestions();
      const finalScore = Math.round(objectiveScore + subjectScore);
      const submit = () => {
