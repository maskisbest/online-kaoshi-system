      ...
      <el-tab-pane name="first">
        ...
        <section class="append">
          <ul>
            ...
            <li v-if="optionValue === '主观题'">
              <span>所属章节：</span>
              <el-input
                placeholder="请输入对应章节"
                v-model="postSubject.section"
                class="w150"
                clearable
              >
              </el-input>
          ...
          <!-- 主观题 -->
          <div class="change subject" v-if="optionValue == '主观题'">
            <div class="title">
              <el-tag>题目:</el-tag><span>输入题目内容</span>
              <el-input
                type="textarea"
                rows="4"
                v-model="postSubject.question"
                placeholder="请输入题目内容"
                resize="none"
                class="answer"
              >
            ...
            <div class="title">
              ...
              ><span>可填写要点或示例答案</span>
              <el-input
                type="textarea"
                rows="4"
                v-model="postSubject.answer"
                placeholder="请输入参考答案（可用分号分隔要点）"
                resize="none"
                class="answer"
              >
              </el-input>
            ...
            <div class="submit">
              <el-button type="primary" @click="subjectSubmit()"
                >立即添加</el-button
              >
            </div>
          </div>
      ...
      <el-tab-pane name="second">
        ...
        <div class="box">
          <ul>
            ...
            <li>
              <span>主观题数量：</span>
              <el-input type="text" v-model="subjectNumber"></el-input>
            </li>
            <li>
...
export default {
  data() {
    return {
      ...
      postPaper: {
        //考试管理表对应字段
        paperId: null,
        questionType: null, // 试卷类型 1--选择题  2--填空题   3--判断题 4--主观题
        questionId: null,
      },
    };
  },
  ...
  methods: {
    ...
    create() {
      if (!this.paperId) {
        this.$message.error("缺少试卷ID，请从试卷列表进入组卷");
        return;
      }
      const multiNumberVal = Number(this.changeNumber) || 0;
      const fillNumberVal = Number(this.fillNumber) || 0;
      const judgeNumberVal = Number(this.judgeNumber) || 0;
      const subjectNumberVal = Number(this.subjectNumber) || 0;
      if (
        multiNumberVal + fillNumberVal + judgeNumberVal + subjectNumberVal <=
        0
      ) {
        this.$message.warning("请至少设置一种题型的数量");
        return;
      }
      const multiNumber = Number(this.changeNumber) || 0;
      const fillNumber = Number(this.fillNumber) || 0;
      const judgeNumber = Number(this.judgeNumber) || 0;
      const subjectNumber = Number(this.subjectNumber) || 0;
      this.$axios({
        url: "/api/item",
        method: "post",
        data: {
          multiNumber: multiNumberVal,
          fillNumber: fillNumberVal,
          judgeNumber: judgeNumberVal,
          subjectNumber: subjectNumberVal,
          paperId: this.paperId,
          subject: "计算机网络", //题目数量太少，指定为计算机网络出题
        },
      }).then((res) => {
        if (res.data.code === 200) {
          this.$message.success("组卷成功，已将题目填入试卷");
        } else {
          this.$message.error(
            res.data.message || "组卷失败，请检查题库数量是否充足"
          );
        }
        console.log(res);
        let data = res.data;
    ...
    changeSubmit() {
      //选择题题库提交
      if (!this.postChange.question || !this.postChange.rightAnswer) {
        this.$message.warning("请填写题目并选择正确选项");
        return;
      }
    ...
    fillSubmit() {
      //填空题提交
      if (!this.postFill.question || !this.postFill.answer) {
        this.$message.warning("请填写题目和正确答案");
        return;
      }
    ...
    judgeSubmit() {
      //判断题提交
      if (!this.postJudge.question || !this.postJudge.answer) {
        this.$message.warning("请填写题目并选择答案");
        return;
      }
    ...
    subjectSubmit() {
      //主观题提交
      if (!this.postSubject.question || !this.postSubject.answer) {
        this.$message.warning("请填写题目和参考答案要点");
        return;
      }
      this.postSubject.subject = this.subject;
      this.$axios({
        url: "/api/subjectQuestion",
        method: "post",
        data: {
          ...this.postSubject,
        },
        ...
        .then(() => {
          this.$axios(`/api/subjectQuestionId`).then((res) => {
            //获取当前题目的questionId
            let questionId = res.data.data.questionId;
            this.postPaper.questionId = questionId;
